// Thank you Larry Bank https://github.com/bitbank2/image_to_c
...
void ImageInfo(FILE *iHandle, int iFileSize, char *szInfo, image *_img)
{
    int i, j, k;
    int iBytes;
...
extern void image2c(char *path)
{
    ...
    char szInfo[256];
    image img;
    ihandle = fopen(path, "rb"); // open input file
    if (ihandle == NULL)
    {
        fprintf(stderr, "Unable to open file: %s\n", path);
        return; // bad filename passed
    }

    fseek(ihandle, 0L, SEEK_END); // get the file size
    iSize = (int)ftell(ihandle);
    fseek(ihandle, 0, SEEK_SET);
    ImageInfo(ihandle, iSize, szInfo, &img); // get image info
    p = (unsigned char *)malloc(0x10000);    // allocate 64k to play with
    GetLeafName(path, szLeaf);
    ...
    fclose(ihandle);
    printf("};\n");
    printf("const image %s = {.height = %d, .width = %d, .pixels = (pixel *)%sPixels};", szLeaf, img.height, img.width, szLeaf);
    printf("\n#endif\n\n"); // final closing brace
    return;
} /* main() */
//
